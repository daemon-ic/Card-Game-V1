export const WNRSMasterData = [
  "What do you think is the hardest part of what I do for a living?",

  "What reality show do you think I'm most likely to binge-watch? Explain.",

  "What's the first thing you noticed about me?",

  "If Myspace was still a thing what would my profile song be?",

  "If you were to buy me a present, knowing nothing about me other than what I look like, what would it be?",

  "On a scale of 1-10, how messy do you think my car is? 1 being the cleanest 10 complete disaster, explain.",

  "Do you think plants thrive or die in my care? Explain.",

  "Do I look kind? Explain.",

  "Do I seem like more of a creative or analytical type? Explain.",

  "Do I seem like someone who would get a name tattooed on myself? Why or why not?",

  "Finish the sentence: Just by looking at you I'd think _____________.",

  "What do my shoes tell you about me?",

  "Do you think I've ever been fired from a job? If so, what for?",

  "What was your first impression of me?",

  "What was your first impression of me on Bumble vs IRL?",

  "How would you describe what you think my type is in three words?",

  "What did you think about my first message?",

  "Between the two of us, who seems like the better texter? Explain.",

  "What does my Instagram tell you about me?",

  "Make an assumption about me",

  "What about me intrigues you?",

  "What do you think I'm most likely to splurge on?",

  "As a child, what do you think I wanted to be?",

  "Finish the sentence: just by looking at you I'd think ...",

  "Do you think I was popular in school? Explain.",

  "How many speeding tickets do you think I've gotten in my life?",

  "Do I seem like someone who would get a name tattooed on myself? Why or why not?",

  "Do you think I fall in love easily why or why not",

  "What compliment do you think I hear the most?",

  "Do you think I ever checked an Ex phone for evidence?",

  "What subject do you think I thrived at in school and did I fail any?",

  "Do I seem like a coffee or tea person - sweetened or unsweetened?",

  "As a child what do you think I wanted to be when I grow up?",

  "Do I seem like a morning person or a night owl?",

  "Do you think I was popular at school? Explain.",

  "Do you think I'm usually early on time or late to events. Explain.",

  "Do I remind you of anyone you know?",

  "What do you think I'm most likely to splurge on?",

  "How many speeding tickets you think I've gotten in my life?",

  "Wildcard: Ask and answer the next question in a different accent.",

  "Wildcard: Create your own question.",

  "Am I someone you typically connect with?",

  "Do I intimidate you? Why or why not?",

  "What's your favorite song lyric that comes to your mind from the top of your head?",

  "As a child, what do you think I wanted to be?",

  "Wildcard: Write down your number one goal for the next month. Compare.",

  "What's been your happiest memory this past year?",

  "Have you changed your mind about anything recently?",

  "What has been your earliest recollection of happiness?",

  "What lesson took you the longest to un-learn?",

  "Are you lying to yourself about anything?",

  "What questions are you trying to answer most in your life right now?",

  "When was the last time you surprised yourself?",

  "What title would you give this chapter in your life?",

  "What do you crave more of?",

  "Finish the sentence: Strangers would describe me as ______ only I know that I am _______",

  "What's the worst pain you have ever been in that wasn't physical?",

  "Has a stranger ever changed your life?",

  "What's your first love's name and the reason you fell in love with them?",

  "What would your younger self not believe about your life today?",

  "How would you describe the feeling of being in love with one word?",

  "What are you more afraid of: failure or success and why?",

  "What is a dream you let go of?",

  "If you could get to know someone in your life on a deeper level who would it be and why?",

  "What's your mom's name and what is the most beautiful thing about her?",

  "What part of your life works what part of your life hurts?",

  "How can you become a better person?",

  "Do you think the image you have of yourself matches the image other people see you as?",

  "Are you missing anyone right now? Do you think they miss you to?",

  "What fast food restaurant do you think I'm most likely to drive through? What's my order?",

  "Do you think I intimidate others? Why or why not?",

  "When you are asked how are you, how often do you answer truthfully?",

  "How are you, really?",

  "What's the most unexplainable thing that happened to you?",

  "What's your father's name? And tell me one thing about him.",

  "If you could have it your way: whom would you be with? Where would you be? What would you be doing?",

  "Describe your perfect day!",

  "What dating advice would you give your younger self?",

  "How old do you feel emotionally?",

  "Will I find your Ex on your Insta feed? Why or why not?",

  "What's the most embarrassing thing that happened to you on a date?",

  "What could you have done better in previous relationships?",

  "Is there an image of yourself you try to project on a first date you wish you could let go of?",

  "Finish the sentence: Dating is ____________ (both players)",

  "What did the people who raised you, teach you about love?",

  "Describe your perfect date.",

  "What has being single taught you about yourself?",

  "What's your least favorite question to be asked on a date? What do you wish you would be asked more?",

  "What are you passionate about?",

  "What are you not currently giving enough time to?",

  "What about my profile made you swipe right?",

  "Wildcard: Write down the three most important things to you in a relationship (30 sec) Compare.",

  "Wildcard: Write down a dating pet peeve (30 sec). Compare.",

  "Wildcard: Draw your current mood. Compare.",

  "Based on what you learned about me what would you recommend I read?",

  "What about me surprised you?",

  "Based on what you know about me, do you have a Netflix recommendation?",

  "What do you think my superpower is?",

  "What do you think our most important similarities are?",

  "What do you think is one thing I could do that would drastically improve my life?",

  "What would be the perfect gift for me?",

  "How would you describe me to a stranger?",

  "What do I need to hear right now?",

  "Based on what you've learned about me, does my social media portray me accurately?",

  "What is a lesson you will take away from our conversation?",

  "What can I help you with?",

  "What do you think do I fear the most?",

  "Where am I most qualified to give advice about?",

  "What about me is the hardest for you to understand?",

  "If we were a band what would be our name?",

  "Admit something.",

  "What parts of yourself do you see in me?",

  "How does one earn your vulnerability?",

  "Dare your partner to do something out of their comfort zone next week.",

  "What do you recommend I should let go of?",

  "What has this conversation taught you about yourself?",

  "What do you think my defining characteristic is?",

  "What question were you most afraid to answer?",

  "Why do you think we met?",

  "When this game is over, what will be something you will remember about me?",

  "What do you think my weakness is?",

  "How do our personalities complement each other?",

  "What do you think I should know about myself that perhaps I'm unaware of?",

  "What would make you feel closer to me?",

  "What do you admire most about me?",

  "In one word, describe how you feel right now.",

  "Do you believe everyone has a calling? If so, do you think I've found mine?",

  "What do you think my defining characteristic is?",

  "What can we create together?",

  "What answer of mine made you light up?",

  "What's the most attractive quality about me that isn't physical?",

  "Wildcard: Based on what you learned re-write each other's dating app bios. (30 sec)",

  "Am I like you expected me to be?",

  "What do you admire most about me?",

  "How would you describe me to a stranger?",

  "Wildcard: Draw your favorite memory with me. Compare.",

  "What do you think I've been binge-watching lately?",

  "What do you think has been my go-to snack during this time?",

  "What time do you think I've been waking up these days?",

  "Do you think I'm mainly cooking or post-mating?",

  "What are you most excited for today? Big or small?",

  "What would you feel best to fully accept in this current moment?",

  "Wildcard. Share your screen time. Both players.",

  "What can you create during this time with exactly what you have? Big or small.",

  "What's the most fun you've had during this time?",

  "What's the most valuable lesson this time has taught you so far?",

  "What's been the hardest part of all of this for you? Permission to vent?",

  "What movie do you wish you'd never seen so you can watch it again for the first time during quarantine?",

  "What's been the kindest thing someone has done for you during this time? What's been the kindest thing you've done for yourself?",

  "WILDCARE. Share your most played song at this moment.",

  "When quarantine is beyond us. What change do you want to make in your day to day life, if any?",

  "If you made a playlist for me, what 3 songs would be on it?",

  "What do I need to watch as soon as I have the time?",

  "How can I be best there for you during this chapter?",

  "WILDCARE. Who's been on your mind lately? Send them a text letting them know you're thinking of them. Both players.",

  "FINAL CARD. Each player writes a virtual message to your partner. Press send once you two have hung up.",

  "Are you afraid to be yourself?",

  "Are you closer to your mom or dad?",

  "Are you proud of yourself?",

  "Can you remember the last time you cried, and what was the reason?",

  "Describe a dream that you've had recently.",

  "Describe your first love.",

  "Do you agree that people never change, they just show their true colors?",

  'Do you believe in "right person, wrong timing"?',

  "Do you believe in fate, or do you think that things happen because of our decisions?",

  "Do you believe in second chances? Why or why not?",

  "Do you believe that the present is really better than in the past?",

  "Do you prefer someone cheering you up, or being left alone when you're in a foul mood?",

  "Do you think I'm a top or bottom?",

  "Do you think that cheating partners should be given a second chance?",

  "Have you ever fallen in love?",

  "Have you experienced losing someone you care a lot about?",

  "How important is trust in a relationship?",

  "How long was your shortest and longest relationship?",

  "How would you rank the following in importance: family, career, love life?",

  "If you could change your name, what would it be?",

  "If you could go back in time, would you change something from your past?",

  "If you could go back in time, would you? Why?",

  "If you could go back to an ex or someone you talked to, would you?",

  "If you could go back to your last relationship, would you?",

  "If you could meet one superhero, who would it be?",

  "If you could, what part of your life would you go back to?",

  "If you could, what would you change about yourself?",

  "If you were offered an all-expense-paid plastic surgery, would you accept it, and what would you change about your physical appearance?",

  "If your friend needed you to lie for them, would you?",

  "Is there anyone you think you've hurt?",

  "Is there anything you regret not doing?",

  'Is there somebody you would consider "the one who got away"?',

  "Is there someone in your life you can always run to?",

  "What are the three most important things to you in a relationship? (30 sec) Compare.",

  "What are the two greatest lessons you've learned from a past relationship?",

  "What are you in denial about?",

  "What breaks your heart?",

  "What do you think I consider my biggest achievement?",

  "What do you think I regret the most?",

  "What do you think is the dumbest thing you've ever done?",

  "What do you think went wrong in your last relationship?",

  "What do you want more than anything in your life?",

  "What do you want to achieve by the time you die?",

  "What emotion would you say you experience the most?",

  "What five lessons have you learned the hard way?",

  "What is one positive and negative thing about me?",

  "What is one thing that you wish you could change about yourself?",

  "What is something that you might need reassurance for?",

  "What is something that you've never told anyone?",

  "What is the most important quality your partner must have?",

  "What is the worst lie you have ever told?",

  "What is your biggest regret?",

  "What is your biggest turn-off?",

  "What is your dating pet peeve? (30 sec). Compare.",

  "What is your number one goal for the next month?",

  "What life-changing event have you experienced?",

  "What makes you the happiest?",

  "What makes you, YOU?",

  "What motivates you to get out of bed?",

  "What questions would you most like the answer to?",

  "What was a major turning point in your life?",

  "What was the lowest point in your life?",

  "What was the most stressful experience of your life?",

  "What would my dating app bio say?",

  "What's one thing you're insecure about?",

  "What's something you love about yourself?",

  "What's something you're thankful for?",

  "What's the best and worst piece of advice you've ever received?",

  "What's your favorite planet?",

  "When was the last time you completely lost yourself?",

  "When was the last time you cried and why?",

  "Where do you feel most like yourself?",

  "Which is the better option: being a heartbreaker or being heartbroken?",

  "Which would you prefer in a romantic partner: a dreamer or an achiever?",

  "Who is someone that has changed your life the most?",

  "Wildcard: Both players share something you're most grateful for in this current moment.",

  "Wildcard: How can I add 1% more happiness in your life?",

  "Wildcard: Press shuffle on your music library. explain the first song that comes up!",

  "Wildcard: Share a favorite memory of yours. compare. (both)",

  "Wildcard: Show the first photo in your camera roll. explain.",

  "Wildcard: Sing your favorite song lyrics you can think of off the top of your head. (both)",

  "Wildcard: Staring contest. first-person to blink must reveal a personal problem and ask the other for advice on how they might handle it.",

  "Wildcard: Sake a selfie together",

  "Wildcard: Tell the other person to close their eyes and to keep them closed. (wait 10 seconds. describe to them in detail your favorite memory of them/ two of you)",

  "Wildcard: Think of something that you dislike that most people don't. on the count of 3 say it out loud.",

  "Wildcard: Think of your favorite childhood tv show of all time, on the count of three say it out loud (both players)",

  "Wildcard: Write down a dating pet peeve (30 sec). Compare.",

  "Wildcard: Write down the three most important things to you in a relationship (30 sec) Compare.",

  "Would you rather be called vain or insecure?",
];
